...
class Session(object):
	...
	if player.get_account_id() in gph.banlist:
		name = player.getName()
		clientID =player.getInputDevice().getClientID()
		bsInternal._chatMessage("You are banned from this server")
...
class GameActivity(Activity):
    ...
    def getMap(self):
        ...
        nightstart=now.replace(hour=5,minute=05)
        nightend=now.replace(hour=5)    
        if(now>nightstart or now<nightend):
             bs.getSharedObject('globals').tint = (0.5,0.7,1)
             bsUtils.animateArray(bs.getSharedObject('globals'), 'ambientColor', 3, {0: (1 * 1.27, 0, 0), 4: (0, 1 * 1.27, 0), 555 * 2: (0, 0, 1 * 1.27), 9 * 3: (1 * 1.27, 0, 0)}, True)
        return self._map
